...
/** Google Flow automation is restricted to its official canonical, legacy, and Labs Flow routes. */
export class GoogleFlowAdapter extends ObservedPlatformAdapter {
  ...
  override isSupportedUrl(url: URL): boolean {
    ...
    // Flow's former `flow.google` alias now redirects to `flow.google.com`.
    // Keep both exact hosts so old bookmarked project URLs and the current application work,
    // while still rejecting similarly named lookalike domains.
    if (url.hostname === "flow.google" || url.hostname === "flow.google.com") return true
  ...
  /** Converts Flow's rendered image or video container into a normalized media output. */
